...
latlon_point_t LatLonUtility::calculateAnyLatLonPoint(screen_point_t released,  screen_point_t mapPixelCenter, screen_point_t offset) const
{
...
//    latlon_point_t big_percent(float((offset.y + released.y) /map_pixel_size.h),  float((offset.x +released.x) /map_pixel_size.w) )  ;
    ...
    screen_point_t diff_pixels( double(  mapPixelCenter.X - ( released.X + offset.X) ) , double( mapPixelCenter.Y - ( released.Y + offset.Y )) );
